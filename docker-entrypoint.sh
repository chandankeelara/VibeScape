...
# On DB_BACKEND=turso the app talks to a remote libSQL instance, so
# there is no local SQLite file to seed. On Cloud Run this branch is
# always taken because the filesystem is ephemeral tmpfs anyway.
if [ "${DB_BACKEND:-sqlite}" = "turso" ] || [ "${DB_BACKEND:-sqlite}" = "libsql" ]; then
    echo "[entrypoint] DB_BACKEND=$DB_BACKEND — skipping local SQLite seed"
else
